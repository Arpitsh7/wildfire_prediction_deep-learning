"""
APAU-Net Training on MNDWS Dataset
- Loss: WBCEDiceLoss (pos_weight=25, WBCE:Dice = 1:1.5)
- Threshold: 0.85 (fixed)
- Post-processing: Remove small blobs (min_area=25 pixels)
- Target metrics: F1: 0.3725, Precision: 0.2968, Recall: 0.5002
"""
...
def load_mndws_data(data_path, train_ratio=0.7, val_ratio=0.15, test_ratio=0.15):
    """Load MNDWS dataset using the custom loader."""
...
class SubsetDataset(torch.utils.data.Subset):
    """Subset dataset with proper indexing."""
...
def main():
    ...
    log_file = os.path.join(base_dir, "training_mndws_new.log")
...
Post-processing: Remove small blobs (min_area={min_area} pixels)
================================================================================
"""
    ...
    log_to_file(log_file, f"  - BCE weight: 1.0")
    ...
    log_to_file(log_file, f"Threshold: {threshold}")
    log_to_file(log_file, f"Min Area: {min_area}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        log_to_file(log_file, f"{epoch:<8} {train_loss:<12.6f} {val_metrics['f1']:<10.6f} {best_f1:<10.6f} {current_lr:<12.2e}")
    ...
    log_to_file(log_file, f"\nThreshold: {threshold}")
    log_to_file(log_file, f"Min Area: {min_area} pixels")
    log_to_file(log_file, f"True Positives:  {test_metrics['tp']}")
    log_to_file(log_file, f"False Positives: {test_metrics['fp']}")
    log_to_file(log_file, f"False Negatives: {test_metrics['fn']}")
    ...
    log_to_file(log_file, "\n" + "="*60)
    log_to_file(log_file, "TARGET COMPARISON")
    log_to_file(log_file, "="*60)
    log_to_file(log_file, f"Target F1:       {target_f1:.4f} | Actual: {test_metrics['f1']:.4f} | Diff: {test_metrics['f1'] - target_f1:+.4f}")
    log_to_file(log_file, f"Target Precision: {target_precision:.4f} | Actual: {test_metrics['precision']:.4f} | Diff: {test_metrics['precision'] - target_precision:+.4f}")
    log_to_file(log_file, f"Target Recall:    {target_recall:.4f} | Actual: {test_metrics['recall']:.4f} | Diff: {test_metrics['recall'] - target_recall:+.4f}")
    
    model_path = os.path.join(checkpoint_dir, "apau_net_mndws.pth")
    ...
    results_path = os.path.join(results_dir, "apau_net_mndws_results.json")
