...
def main():
    ...
    log_file = os.path.join(base_dir, "training_ndws_wbce_tversky.log")
    ...
    print(f"""
================================================================================
     NDWS TRAINING: WBCETverskyLoss (pos_weight=3, alpha=0.3, beta=0.7)
                           {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
================================================================================
Device: {device}
Dataset: NDWS filtered (fire patches only)
Loss: WBCETverskyLoss
  - pos_weight: 3
  - alpha: 0.3 (FP penalty)
  - beta: 0.7 (FN penalty)
  - w_bce: 1.0, w_tversky: 1.0
...
"""
    ...
    criterion = WBCETverskyLoss(pos_weight=pos_weight, alpha=alpha, beta=beta)
    optimizer = optim.Adam(model.parameters(), lr=1e-4)
    scheduler = StepLR(optimizer, step_size=5, gamma=0.5)
    
    log_to_file(f"\nLoss: WBCETverskyLoss")
    log_to_file(f"  - pos_weight: {pos_weight}")
    log_to_file(f"  - alpha: {alpha}")
    log_to_file(f"  - beta: {beta}")
    ...
    model_path = os.path.join(checkpoint_dir, "ndws_wbce_tversky_apau_net.pth")
    ...
    results_path = os.path.join(results_dir, "ndws_wbce_tversky_results.json")
